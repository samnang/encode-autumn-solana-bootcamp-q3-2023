...
export async function deployGreetAccount(
  ...
  let greetedPubkey = await PublicKey.createWithSeed(payer.publicKey, GREETING_SEED, programId);
  ...
  const greeting = borsh.deserialize(GreetingSchema, accountInfo.data) as GreetingAccount;
